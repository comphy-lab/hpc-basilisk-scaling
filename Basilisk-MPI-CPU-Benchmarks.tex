...
\section{Existing scaling evidence}
...
\section{Verification and kernel coverage}
...
The stock kernels extend the rank range beyond the representative application.
The two-dimensional full-quadtree problem at $L=14$ contains $2^{28}$ cells;
on Snellius, its Poisson time continues to decrease through 8,192 ranks and
then plateaus at 16,384 ranks. The three-dimensional full-octree problem at
$L=9$ contains $2^{27}$ cells. Its Snellius Poisson time falls from
$19.34$~s at two ranks to $0.0333$~s at 16,384 ranks, with little reduction
beyond 4,096 ranks. The corresponding Curie and Occigen tables distributed
with the stock tests provide independent reference measurements.
...
The active-drop figure then tests how the complete two-phase workload shifts
that limit as resolution and interface count increase.
