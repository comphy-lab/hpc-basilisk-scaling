...
Newtonian drop impact---are timed at $L=9$, $10$ and $11$ over 2--768 ranks.
...
$1024^2$ and $2048^2$ cells) over 2--768 MPI ranks, one to four Snellius
Genoa nodes. Colour encodes $N_x=2^L$, as in
Figure~\ref{fig:application-scaling}(a). The two three-dimensional kernels
remain one-node sweeps at $L=7$ ($128^3$ cells, 2--192 ranks). The timed
window is the incompressible two-phase solver only. Geometric setup, and the
serial dump used to seed the two cases whose interface comes from
\texttt{distance.h}, sit outside that window. The MPI binaries write no
checkpoints. The $L=11$ axisymmetric series is recorded from eight ranks.
...
$1.90$~s at eight ranks to $0.0341$~s at 768 ranks.
...
ideal line to $0.0339$~s at 768 ranks.
